...
import { Role, USER_VALIDATION } from '@ratehub/shared';
...
export class CreateUserDto {
  ...
  @ApiProperty({
    ...
    example: 'admin.user@storerating.local',
  })
  ...
  @ApiProperty({
    ...
    example: '742 Evergreen Terrace, Platform HQ, Springfield',
  ...
  @ApiProperty({
    description: 'User role (allowed values: normal, admin)',
    enum: [Role.NORMAL, Role.ADMIN],
    example: Role.ADMIN,
  })
  @IsNotEmpty({ message: 'Role is required' })
  @IsIn([Role.NORMAL, Role.ADMIN, 'normal', 'admin'], {
    message: 'Role must be either normal or admin',
  })
  role: Role | 'normal' | 'admin';
}
